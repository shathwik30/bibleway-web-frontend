...
export default function ApiTestPage() {
  ...
  const quickLogin = async () => {
    ...
    try {
      // For fetch API, we bypass fetchAPI wrapper because it might attach old tokens
      const res = await fetch(`${process.env.NEXT_PUBLIC_API_URL || "http://localhost:8000/api/v1"}/accounts/login/`, {
        method: "POST",
      ...
      if (res.ok && data.data?.access) {
        ...
        checkToken();
        setResponse({ message: "Successfully logged in to Production!", ...data });
      } else {
        ...
        <div className="flex justify-between items-center mb-10">
          <div>
            ...
            <p className="text-on-surface-variant font-medium">
              Hitting Production Backend: 
              <span className="font-mono text-primary ml-2 bg-primary/10 px-2 py-1 rounded">
                https://bibleway-backend-production.up.railway.app/api/v1
              </span>
            </p>
          </div>
        </div>
          ...
          <div className="w-full lg:w-1/3 space-y-4">
            ...
            <div className="bg-tertiary-fixed/20 p-4 rounded-xl border border-tertiary-fixed-dim/30 mb-6">
              ...
              <p className="text-[10px] text-on-surface-variant mt-2 text-center">
                Note: Since we switched to Railway, your local users won't work. You must use a production account.
              </p>
            </div>
